...
const MyEnrollments = () => {
    ...
    const getCourseProgress = async () => {
        try {
            // const token = await getToken(); // REMOVE Clerk token

            // Use Promise.all to handle multiple async operations
            const tempProgressArray = await Promise.all(
                enrolledCourses.map(async (course) => {
                    // Use apiRequest helper
                    // Ensure the route is correct (GET and uses params)
                    const { data } = await apiRequest(
                        `/user/progress/${course._id}`, 
                        'GET' // Assuming it's a GET request now
                    );

                    // Calculate total lectures
                    let totalLectures = calculateNoOfLectures(course);

                    // Check if lectureCompleted exists on progressData
    ...
    useEffect(() => {
        // Fetch enrollments if user is logged in (using user from context)
        if (userData) {
    ...
    useEffect(() => {
        // Fetch progress if enrollments are loaded
        ...
        <>
            <div className='md:px-36 px-8 pt-10'>
                ...
                {enrolledCourses.length === 0 ? (
                    <p className="mt-10 text-center text-gray-500">You haven't enrolled in any courses yet.</p>
                ) : (
                                ...
                                <tr key={index} className="border-b border-gray-500/20">
                                    <td className="md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3 ">
                                        ...
                                        <div className='flex-1'>
                                            ...
                                            <Line className='bg-gray-300 rounded-full' strokeWidth={2} percent={progressArray[index] && progressArray[index].totalLectures > 0 ? (progressArray[index].lectureCompleted * 100) / progressArray[index].totalLectures : 0} />
                                        </div>
                                    </td>
